...
TNoA *criaArvore(char entrada[100], int tamanho) {
  ...
  if ((tamanho > 0) && (entrada[0] != '*')) {
    ...
    // divide a entrada em duas partes e chama a função criaArvore
    // recursivamente
    i = 1; // inicio da subarvore esquerda na string entrada
    j = 0; // cursor da nova string de entrada que conterá apenas a subárvore
           // desejada
    while (i <= tamanho) {
...
int main(void) {
   ...
   * solicitada no exercício depois imprime a árvore resultante Ela NÃO DEVE SER
   * MODIFICADA
   * */
  TNoA *raiz = NULL;
  int tam;
